...
- (void)didPan:(UIPanGestureRecognizer *)panGesture
{
    if (_scrollEnabled)
    {
        switch (panGesture.state)
        {
            ...
            {
                if (_isDrag) {
                    _movedView = [self itemViewAtPoint:[panGesture locationInView:_contentView]];
                    _originalFrame = _movedView.frame;
                }
                else{
                    _dragging = YES;
                    _scrolling = NO;
            ...
            case UIGestureRecognizerStateEnded:
            {
                if (_isDrag) {
                    if (_moveItem) {
                        if (self.delegate && [self.delegate respondsToSelector:@selector(carousel:itemMoveWithIndex:)]) {
                            [self.delegate carousel:self itemMoveWithIndex:[self indexOfItemView:_movedView]];
                        }
                    }
                    else{
                        [_movedView setFrame:_originalFrame];
                    }
                }
            }
            ...
            default:
            {
                if (_isDrag) {
                    ...
                    CGPoint translatedPoint = [(UIPanGestureRecognizer*)panGesture translationInView:self];
                    CGPoint panGestureMidpoint = [panGesture locationInView:[self superview]];
                    
                    _movedView.center = CGPointMake(_movedView.center.x + translatedPoint.x, _movedView.center.y + translatedPoint.y);
                    [(UIPanGestureRecognizer*)panGesture setTranslation:CGPointMake(0,0) inView:self];
                    
                    if (self.tag == CAROUSEL_TOP) {
                        // check if to move down or not
                        if (_movedView.frame.origin.y > _originalFrame.origin.y + 200) {
                            _moveItem = YES;
                        }
                        else{
                            _moveItem = NO;
                    ...
                    else if(self.tag == CAROUSEL_BOTTOM){
                        //check if to move up or not
                        if (_movedView.frame.origin.y < _originalFrame.origin.y - 200) {
                            _moveItem = YES;
                        }
                        else{
                    ...
                    if (panGestureMidpoint.x < 160) {
                        _isCardInsertedLeft = YES;
                    }
                    else{
                        _isCardInsertedLeft = NO;
                    }
                }
                else{
